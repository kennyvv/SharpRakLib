	...
	public class Session
	{
			...
			switch (data[0])
			{
				case JRakLibPlus.IdOpenConnectionRequest1:
					if (_state == Connecting1 || _state == Connecting2)
					{
						...
						_mtu = req1.NullPayloadLength;

						var reply1 = new OpenConnectionReply1Packet();
						reply1.MtuSize = (ushort) _mtu;
				...
				case JRakLibPlus.IdOpenConnectionRequest2:
					if (_state == Connecting2)
					{
						...
						if (_server.PortChecking && req2.ServerAddress.GetPort() != _server.BindAddress.Port)
						{
		...
		private void HandleSplitPacket(EncapsulatedPacket pk)
		{
			...
			lock (_splitQueue)
			{
				...
				{
					var ep = new EncapsulatedPacket();
					IBuffer b = JavaByteBuffer.Allocate(1024*1024, false);
					var size = 0;
					var packets = _splitQueue[pk.SplitId];
					for (var i = 0; i < pk.SplitCount; i++)
					{
						b.Put(packets[i].Payload);
						size = size + packets[i].Payload.Length;
					}
					var data = new byte[size];
					var a = b.ToByteArray();

					Array.Copy(a, 0, data, 0, size);
			...
			switch (pk.Payload[0])
			{
				...
				case JRakLibPlus.McClientHandshake:
					if (_server.PortChecking)
					{
						...
						if (chp.Address.GetPort() != _server.BindAddress.Port)
						{
				...
				case JRakLibPlus.McPong:
					...
					_lastPing = (int) (JavaHelper.CurrentTimeMillis() - pong2.PingId);
					break;
